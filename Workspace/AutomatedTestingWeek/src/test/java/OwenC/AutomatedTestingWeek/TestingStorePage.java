...
public class TestingStorePage {
	...
	public void registerAccountTest() {
		...
		String gender = inputList.get(1).toLowerCase();
		...
		{
			accountPage.getRdbGender1().click();
		} else {
			accountPage.getRdbGender2().click();
		}
		
		accountPage.getTxtFirstName().sendKeys(firstName);
		accountPage.getTxtLastName().sendKeys(lastName);
		accountPage.getTxtEmail().clear();
		...
		Select select = new Select(accountPage.getDdbDayOfBirth());
		select.selectByValue(dobSplit[0]);
		select = new Select(accountPage.getDdbMonthOfBirth());
		select.selectByValue(dobSplit[1]);
		accountPage.getDdbYearOfBirth().sendKeys(dobSplit[2] + "\n");
		...
		accountPage.getTxtPostCode().sendKeys(zip.substring(0, zip.length() - 2));
		...
		test.log(Status.INFO, "Account details have been added");
